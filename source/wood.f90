module wood
	...
	integer, allocatable :: G(:,:), forest_glob(:,:), G_forest(:,:)
	integer :: inf, k, timer, knots
	
	contains

	subroutine search_forest(G_in, k_in, G_forest_out)	
		...
		do i = 1, n
			forest_in(i,i) = 0
			forest_in(i,n + 1) = i
			min = minloc(G_in(i,:), 1, G_in(i,:) >= 0)

			! Записываем минимальные веса возможных деревьев
			if (min /= 0) then
				min_arg(1,i) = G_in(i,min)
			else
				min_arg(1,i) = inf
			endif
			min_arg(2,i) = i
		...
		do i = 1, size(forest_glob,1)
			do ii = 1, knots
				if (forest_glob(i,ii) > 0) then
					G_forest_out(ii,forest_glob(i,ii)) = G_in(ii,forest_glob(i,ii))
				endif
			enddo
		enddo
	...
	recursive subroutine forest_help(forest, min_arg)
		...
		integer, allocatable :: G_tmp(:,:), G_tmp_t(:,:), forest_out(:,:), min_arg_out(:,:), tree(:,:), min_arg(:,:), b(:) 
		...
		do i = 1, tmp
			...
			! Строим минимальное остовное дерево из корня i
			if (check(G_tmp_t, i)) then
				call chinese(G_tmp_t, i, tree)
				tree = transpose(tree)
		...
		if (check(G_tmp_t, root_b)) then
			 
			call chinese(G_tmp_t, root_b, tree)
		endif
